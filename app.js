...
class MillionaireApp {
    ...
    handleLogin() {
        ...
        if (!username || !password) {
            this.ui.showNotification('الرجاء ملء جميع الحقول', 'error');
            return;
        }
    ...
    showAdminLogin() {
        const modalContent = `
            ...
            <div class="form-group">
                ...
                <input type="password" id="admin-password" class="form-control" value="admin123">
            </div>
            <div class="form-group">
    ...
    startNewGame() {
        const result = this.game.startNewGame({
            player: this.auth.getCurrentUser()?.username,
            difficulty: 'medium',
            categories: ['general'],
            timerEnabled: true
        });
    ...
    // تحديث رصيد المستخدم (للوحة الإدارة)
    ...
    editSubscription(planType) {
        const plans = {
            free: { price: 0, name: 'مجاني' },
            pro: { price: 9.99, name: 'برو' },
            premium: { price: 19.99, name: 'فخمة' }
        };
